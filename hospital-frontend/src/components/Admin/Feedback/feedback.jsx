...
const Feedback = () => {
  ...
  const handleSubmitFeedback = async () => {
    try {
      ...
      console.log("Submitting feedback form with token:", token);
      ...
      const formData = await formRes.json();
      console.log("Feedback form response:", formData);
      ...
      if (formData.success && formData._id) {
        ...
        console.log("Submitting feedback details:", feedbackDetails);
        ...
        for (const detail of feedbackDetails) {
          ...
          const detailRes = await fetch(FEEDBACK_DETAIL_API, {
            method: "POST",
            headers: getAuthHeaders(),
            body: JSON.stringify(detailPayload)
          });
          ...
          if (!detailData.success) {
            console.error("Error submitting detail:", detailData);
          }
        }
        ...
        console.error("Feedback form submission failed:", formData);
        alert(formData.message || "Error submitting feedback form. Please try again.");
      }
    } catch (err) {
